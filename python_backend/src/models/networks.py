...
from src.models.encoders.helpers import (
    get_block,
    Flatten,
    bottleneck_IR,
    bottleneck_IR_SE,
...
class LocalMLP(nn.Module):
    ...
    def __init__(
        self, dim_component=512, dim_style=512, num_w_layers=18, latent_squeeze_ratio=1
    ):
        ...
        self.mlp = nn.Sequential(
            ...
            nn.LeakyReLU(),
            EqualLinear(
                dim_style // latent_squeeze_ratio, dim_style * num_w_layers, lr_mul=1
            ),
        )
...
class Net3(nn.Module):
    ...
    def __init__(
        ...
        for i in range(self.opts['num_seg_cls']):
            self.MLPs.append(
                LocalMLP(
                    ...
                    dim_style=512,
                    num_w_layers=self.remaining_layer_idx
        ...
        if self.remaining_layer_idx != 17:
            for param in self.G.convs[-(17 - self.remaining_layer_idx) :].parameters():
                param.requires_grad = False
            for param in self.G.to_rgbs[
                -(17 - self.remaining_layer_idx) // 2 - 1 :
            ].parameters():
                param.requires_grad = False

    def forward(
        self, img, mask, resize=False, randomize_noise=True, return_latents=False
    ):
        codes_vector, structure_feats = self.encoder(
            F.interpolate(img, (256, 256), mode='bilinear'), mask
        )  # [bs,#seg_cls, D], [bs,C,32,32]
        ...
        if self.opts['start_from_latent_avg']:
            if self.opts['learn_in_w']:
                ...
                codes = codes + self.latent_avg[: self.remaining_layer_idx, :].repeat(
                    codes.shape[0], codes.shape[1], 1
                )
                remaining_codes = self.latent_avg[self.remaining_layer_idx :, :].repeat(
                    bs, num_comp, 1
                )
                ...
                if self.remaining_layer_idx != 17:
                    codes = codes + self.latent_avg[
                        : self.remaining_layer_idx, :
                    ].repeat(codes.shape[0], codes.shape[1], 1, 1)
                    remaining_codes = self.latent_avg[
                        self.remaining_layer_idx :, :
                    ].repeat(bs, num_comp, 1, 1)
                    codes = torch.cat([codes, remaining_codes], dim=2)
                else:
                    codes = codes + self.latent_avg.repeat(
                        codes.shape[0], codes.shape[1], 1, 1
                    )

        # G(w)
        images1, result_latent, structure_feats_GT = self.G(
            [codes],
            structure_feats,
            mask,
    ...
    def get_style_vectors(self, img, mask):
        ...
        """
        style_vectors, structure_feats = self.encoder(
            F.interpolate(img, (256, 256), mode='bilinear'), mask
        )  # [bs,#seg_cls, D], [bs,C,32,32]
        ...
        if self.opts['start_from_latent_avg']:
            if self.opts['learn_in_w']:
                ...
                if self.remaining_layer_idx != 17:
                    ...
                    style_codes = codes + self.latent_avg.repeat(
                        codes.shape[0], codes.shape[1], 1, 1
                    )
    ...
    def gen_img(
        ...
        images, result_latent, structure_feats = self.G(
            [style_codes],
            struc_codes,
            mask,
            input_is_latent=True,
