...
from src.models.stylegan2.op import (
    FusedLeakyReLU,
    fused_leaky_relu,
    upfirdn2d,
    conv2d_gradfix,
...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, input, style):
        ...
        )  #  [1, C_out, C_in, kh, kw]  * [bs,1,512,1,1]
...
class StyledConv(nn.Module):
    ...
    def forward(self, input, style, mask, noise=None):
        if not self.mask_op:
            ...
            # style modulation & demodulation for each component
            bs, _, H, W = input.size()
            H_out, W_out = (H * 2, W * 2) if self.conv.upsample else (H, W)

            segmap = F.interpolate(mask, size=(H_out, W_out), mode='nearest')
            comp_aware_out = torch.zeros(
                bs, self.conv.out_channel, H_out, W_out, device=input.device
            )

            num_comp = style.size(1)  # [bs,#seg_cls, #dim]
            for comp_i in range(num_comp):
                out_i = self.conv(input, style[:, comp_i, :])
                masked_out_i = out_i * segmap[:, comp_i, :, :].unsqueeze(dim=1)
                comp_aware_out += masked_out_i
            # ===============================================================
            out = comp_aware_out
...
class ToRGB(nn.Module):
    ...
    def forward(self, input, style, mask, skip=None):
        if not self.mask_op:
            out = self.conv(input, style)
        else:
            # ============= Mask-guided style injection ========================
            # style modulation & demodulation for each component
            # style with shape: [bs, #seg_cls, #dim]
            bs, _, H, W = input.size()
            segmap = F.interpolate(
                mask, size=(H, W), mode='nearest'
            )  # one-hot, [bs,#seg_cls,H,W]
            comp_aware_out = torch.zeros(bs, 3, H, W, device=input.device)
...
class Generator(nn.Module):
    def __init__(
        ...
        self.to_rgb1 = ToRGB(self.channels[4], style_dim, upsample=False, mask_op=True)
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.to_rgbs.append(
                ToRGB(
                    ...
                    and i >= (2 + self.remaining_layer_idx // 2)
                    else True,  # 这里一定是大于等于
                    # mask_op = True
                )
            )
    ...
    def forward(
        ...
        skip = self.to_rgb1(out, latent[:, :, 1], mask)
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            ...
            if i < self.remaining_layer_idx:
                ...
                print('1111111112223', out.size(), latent.size())
        ...
        if return_latents:
            ...
            return image, None, intermediate_feats
