...
from src.pretrained.face_vid2vid.modules.generator import (
    OcclusionAwareGenerator,
    OcclusionAwareSPADEGenerator,
)
from src.pretrained.face_vid2vid.modules.keypoint_detector import (
    HEEstimator,
    KPDetector,
)
...
def keypoint_transformation(
    kp_canonical, he, estimate_jacobian=True, free_view=False, yaw=0, pitch=0, roll=0
):
...
def make_animation(
    ...
    with torch.no_grad():
        ...
        kp_driving_initial = keypoint_transformation(
            kp_canonical, he_driving_initial, estimate_jacobian
        )
...
# ==============================================
def init_facevid2vid_pretrained_model(cfg_path, ckpt_path, cpu=False):
    generator, kp_detector, he_estimator = load_checkpoints(
        config_path=cfg_path, checkpoint_path=ckpt_path, gen="spade", cpu=cpu
    )
...
def drive_source_demo(
    source_im,
    target_ims,  # input paths
    generator,
    kp_detector,
    he_estimator,
    estimate_jacobian,  # model params
    cpu=False,
):
    """
