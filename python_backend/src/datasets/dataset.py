...
def __ffhq_masks_to_faceParser_mask_detailed(mask):
    ...
    eyebrows = np.logical_or(np.equal(mask, 2), np.equal(mask, 3))
...
def __ffhq_masks_to_faceParser_mask(mask):
    ...
    mouth = np.logical_or(
        np.logical_or(np.equal(mask, 11), np.equal(mask, 12)), np.equal(mask, 13)
    )
...
def __celebAHQ_masks_to_faceParser_mask_detailed(celebA_mask):
    ...
    print('??????', celebA_mask.shape, converted_mask.shape)
...
class CelebAHQDataset(Dataset):
    def __init__(
        ...
        if mode == "train":
            ...
            self.labels = sorted(
                [
                    osp.join(self.root, "CelebA-HQ-mask", "%d.png" % idx)
                    for idx in range(28000)
                ]
            )
            self.labels_vis = (
...
class FFHQDataset(Dataset):
    def __init__(
        ...
        self.imgs = sorted(
            [
                osp.join(self.root, "images_1024", line.replace("\n", ""))
                for line in f_lines
            ]
        )
        self.imgs = self.imgs[: int(len(self.imgs) * self.fraction)]
