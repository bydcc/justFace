...
def paste_image_mask(inverse_transform, image, dst_image, mask, radius=0, sigma=0.0):
    ...
    if radius != 0:
        ...
        eroded = cv2.erode(
            mask_np, kernel, borderType=cv2.BORDER_CONSTANT, borderValue=255
        )
    ...
    projected = image_masked.transform(
        dst_image.size, Image.PERSPECTIVE, inverse_transform, Image.BILINEAR
    )
...
def faceSwapping_pipeline(
    ...
    source_and_target_files = [
        (os.path.basename(f).split('.')[0], f) for f in source_and_target_files
    ]
    result_name = "swap_%s_to_%s.png" % (
        source_and_target_files[0][0],
        source_and_target_files[1][0],
    )
    ...
    S_256, T_256 = [
        resize(np.array(im) / 255.0, (256, 256)) for im in [S, T]
    ]  # 256,[0,1] range
    T_mask = (
        faceParsing_demo(
            faceParsing_model, T, convert_to_seg12=True, model_name=opts.faceParser_name
        )
    ...
    predictions = drive_source_demo(
        S_256, [T_256], generator, kp_detector, he_estimator, estimate_jacobian
    )
    ...
    drivens = [
        GPEN_demo(pred[:, :, ::-1], GPEN_model, aligned=False) for pred in predictions
    ]
    ...
    if verbose:
        D.save(
            os.path.join(
                save_dir,
                "D_%s_to_%s.png"
                % (source_and_target_files[0][0], source_and_target_files[1][0]),
            )
        )

    # (2) mask of D
    D_mask = faceParsing_demo(
        faceParsing_model, D, convert_to_seg12=True, model_name=opts.faceParser_name
    )
    if verbose:
    ...
    if verbose:
        ...
        driven_face, _, structure_feats = net.gen_img(
        ...
        target_face, _, structure_feats = net.gen_img(
            torch.zeros(1, 512, 32, 32).to(opts.device),
            target_style_codes,
            target_onehot,
        )
    ...
    if verbose:
        ...
        torch_utils.tensor2map(swappped_one_hot[0]).save(
            os.path.join(save_dir, "swappedMaskVis.png")
        )
    ...
    if verbose:
        torch.save(
            swapped_style_vectors, os.path.join(save_dir, "swapped_style_vec.pt")
        )
    ...
    swapped_face, _, structure_feats = net.gen_img(
        torch.zeros(1, 512, 32, 32).to(opts.device), swapped_style_codes, swapped_onehot
    )
    ...
    outer_dilation = 5
    mask_bg = logical_or_reduce(
        *[swapped_msk == clz for clz in [0, 11, 4]]
    )  # For face swapping in video，condisder 4,8,7 as part of background.  11 earings 4 hair 8 neck 7 ear
    ...
    if opts.lap_bld:
        content_mask, border_mask, full_mask = create_masks(
            foreground_mask, outer_dilation=outer_dilation, operation='expansion'
        )
    else:
        content_mask, border_mask, full_mask = create_masks(
    ...
    if opts.lap_bld:
        ...
        if outer_dilation == 0:
            swapped_and_pasted = smooth_face_boundry(
                swapped_face_image, T, content_mask_image, radius=outer_dilation
            )
        else:
            swapped_and_pasted = smooth_face_boundry(
                swapped_face_image, T, full_mask_image, radius=outer_dilation
            )
    ...
    if only_target_crop:
        ...
        projected = swapped_and_pasted.transform(
            orig_image.size, Image.PERSPECTIVE, inv_trans_coeffs, Image.BILINEAR
        )
        pasted_image.alpha_composite(projected)
    elif need_crop:
...
if __name__ == "__main__":
    ...
    (
        ...
        he_estimator,
        estimate_jacobian,
    ) = init_facevid2vid_pretrained_model(face_vid2vid_cfg, face_vid2vid_ckpt)
    ...
    if opts.faceParser_name == "default":
        ...
        raise NotImplementedError(
            ...
            "the current supported models are [ default | segnext ], but %s is given."
            % opts.faceParser_name
        )

    faceParsing_model = init_faceParsing_pretrained_model(
        opts.faceParser_name, faceParser_ckpt, config_path
    )
